...
define("text", [ "module" ], function(module) {
    ...
    text.get = function(url, callback, errback) {
        ...
        masterConfig.onXhr && masterConfig.onXhr(xhr, url), xhr.onreadystatechange = function() {
            ...
            4 === xhr.readyState && (status = xhr.status || 0, status > 399 && 600 > status ? (//An http 4xx or 5xx error. Signal an error.
            err = new Error(url + " HTTP status: " + status), err.xhr = xhr, errback && errback(err)) : callback(xhr.responseText), 
        ...
        try {
            for (stringBuffer = new java.lang.StringBuffer(), line = input.readLine(), // Byte Order Mark (BOM) - The Unicode Standard, version 3.0, page 324
            // http://www.unicode.org/faq/utf_bom.html
            // Note that when we use utf-8, the BOM should appear as "EF BB BF", but it doesn't due to this bug in the JDK:
            // http://bugs.sun.com/bugdatabase/view_bug.do?bug_id=4508058
            line && line.length() && 65279 === line.charAt(0) && (// Eat the BOM, since we've already found the encoding on this file,
            // and we plan to concatenating this buffer with others; the BOM should
            // only appear at the top of a file.
            line = line.substring(1)), null !== line && stringBuffer.append(line); null !== (line = input.readLine()); ) stringBuffer.append(lineSeparator), 
    ...
    return '<div class="campaign-result<% if(featured) { %> big<% } %>">\n  <% if(staffPick) {  %><div class="flag staff-pick">Staff Pick</div><% } %>\n  <img src="data:image/gif;base64,R0lGODlhAQABAIAAAAUEBAAAACwAAAAAAQABAAACAkQBADs=" data-src="<%= image %>">\n  <div class="headline">\n    <h3><%= title %></h3>\n    <p><%= description %></p>\n  </div>\n  <a class="full-link" href="<%= url %>"><span><%= title %></span></a>\n</div>\n';
   ...
   */
     ...
     */
        init: function($div, $blankSlateDiv, paginateAction, clearFormAction) {
            ResultsView.$div = $div, ResultsView.$div.hide(), ResultsView.$paginationLink = $("<div class='pagination-link'><a href='#' class='secondary js-finder-show-more'>Show More</a></div>"), 
            $("body").on("click", ".js-finder-show-more", function(event) {
                event.preventDefault(), paginateAction(ResultsView.start);
            }), ResultsView.$blankSlateDiv = $blankSlateDiv, ResultsView.$div.on("click", "#reset-filters", function(event) {
     ...
     */
        showBlankSlate: function() {
            ResultsView.$div.hide(), ResultsView.$blankSlateDiv.show();
        },
        /**
     ...
     */
        showEmptyState: function() {
            ...
            ResultsView.$div.append(message);
        },
        /**
     ...
     */
        showPaginationLink: function(showLink) {
            ResultsView.$paginationLink.remove(), showLink && (ResultsView.$div.append(ResultsView.$paginationLink), 
     ...
     */
        loading: function(setTo) {
            ...
            ResultsView.$div.append("<div class='spinner'></div>"), ResultsView.$div.toggleClass("loading", setTo);
        },
        /**
     * Have we run the init method?
     */
        checkInit: function() {
            if (null === ResultsView.$div) // Nope. I'm gonna vomit...
     ...
     */
        clear: function() {
            ResultsView.checkInit(), ResultsView.$div.empty(), ResultsView.$div.show(), ResultsView.$blankSlateDiv.hide(), 
            ResultsView.slots = 0, ResultsView.start = 0;
        },
        /**
     ...
     */
        add: function(campaign) {
            ...
            ResultsView.$div.append(campaign.render()), ResultsView.slots++, // Track where we are for paging
   ...
   */
     ...
     **/
        ...
        defaultQuery: [ "fq=bundle:[campaign TO campaign_group]", //"fq=ss_field_search_image:[* TO *]",
        "wt=json", "indent=false", "facet=true", "facet.field=fs_field_active_hours", "facet.field=im_field_cause", "facet.field=im_field_action_type", "sort=bs_field_staff_pick+desc", "rows=8", "fl=label,tid,im_vid_1,sm_vid_Action_Type,tm_vid_1_names,im_field_cause,im_vid_2,sm_vid_Cause,tm_vid_2_names,im_field_tags,im_vid_5,sm_vid_Tags,tm_vid_5_names,fs_field_active_hours,sm_field_call_to_action,bs_field_staff_pick,ss_field_search_image_400x400,ss_field_search_image_720x720,url" ],
     ...
     */
        buildQuery: function(query, q) {
            ...
            return query[pos] = q.length > 0 ? 1 === q.length ? "q=" + q[0] : "q=(" + q.join(") AND (") + ")" : "q=*:*", 
            query = query.join("&"), SolrAdapter.baseURL + SolrAdapter.collection + "/select?" + query + "&rows=10";
        },
        /**
    ...
    return "<div class=\"messages error\">Ooof! We're not sure what's up? Maybe it's us, or it could be your internet connection. Want to try again in a few minutes?</div>\n";
     ...
     */
        init: function($formDiv, $resultsDiv, $blankSlateDiv) {
            ...
            FormView.init($formDiv, Finder.query), ResultsView.init($resultsDiv, $blankSlateDiv, Finder.query, FormView.clear), 
...
define("neue/modal", [ "require", "./events" ], function(require) {
    var $ = window.jQuery, Modernizr = window.Modernizr, Events = require("./events"), modalIsOpen = !1, $modal = null, $modalContent = null, $reference = null, closeable = !1, isOpen = function() {
        return modalIsOpen;
    }, _openHandler = function(event) {
        event.preventDefault();
        var href = "";
        $(this).data("cached-modal") ? // Preferred method: We load the modal specified in the `data-cached-modal` attribute.
        ...
        href = $($(this).data("cached-modal")) : "#" === event.target.hash.charAt(0) && (// We find the modal based on the ID in the link"s `href`. For example,
        // `<a class="js-modal-link" href="#modal--faq">Click me</a>` would open
        // `<div id="modal--faq"></div>`.
        href = $(event.target.hash)), open(href);
        ...
        switch (id ? (// Save ID of modal for future reference
        $reference = "#" + id, // Set URL hash in the browser
        window.location.hash = "#" + id) : $reference = "", // If Google Analytics is set up, we fire an event to track that a
        // modal has been opened.
        "undefined" != typeof _gaq && null !== _gaq && _gaq.push([ "_trackEvent", "Modal", "Open", $reference, null, !0 ]), 
        modalIsOpen ? (// modal is already open, so just replace current content
        $modalContent.removeClass(), $modalContent.addClass("modal-content"), $modalContent.addClass($el.attr("class")), 
        $modalContent.html($($el).html())) : (// create modal in DOM
        $modal = $('<div class="modal"></div>'), $modalContent = $("<div></div>"), $modal.append($modalContent), 
        $modalContent.html($el.html()), // set up overlay and show modal
        $("body").addClass("modal-open"), $("body").append($modal), options.animated && Modernizr.cssanimations && ($modal.addClass("fade-in"), 
        $modalContent.addClass("fade-in-up")), // copy classes from modal source
        $modalContent.removeClass(), $modalContent.addClass("modal-content"), $modalContent.addClass($el.attr("class")), 
        $modal.show(), // Bind events to close Modal
        $modal.on("click", ".js-close-modal", _closeHandler), $modal.on("click", _closeHandler), 
        modalIsOpen = !0, //  **This fixes an issue with `position:fixed` and the virtual keyboard
        ...
        /iPhone|iPad|iPod/i.test(window.navigator.userAgent) && setTimeout(function() {
            $modal.css({
                position: "absolute",
                overflow: "visible",
          ...
          case "skip":
            ...
            var $skipForm = $(options.skipForm), $skipLink = $("<a href='#' class='js-close-modal modal-close-button -alt'>skip</a>");
            $modalContent.prepend($skipLink), $skipLink.on("click", function(event) {
                event.preventDefault(), $skipForm.submit();
            }), closeable = !1;
            // cannot close modal by clicking background
            break;
        ...
        closeClass && $modalContent.find(".js-close-modal").addClass(closeClass), // We provide an event that other modules can hook into to perform custom functionality when
        ...
        var $messages = $(".messages"), $messagesClone = $modalContent.find(".js-messages-clone");
        $messagesClone && $messages.length && ($messagesClone.addClass("modal-messages"), 
        $messagesClone.html($messages[0].outerHTML), $messagesClone.find(".js-close-message").remove());
        ...
        // Default arguments
        animated = "undefined" != typeof animated ? animated : !0, // Remove URL hash for modal from browser
        window.location.hash === $reference && (window.location.hash = "/"), // If Google Analytics is set up, we fire an event to track that a
        // modal has been closed.
        "undefined" != typeof _gaq && null !== _gaq && _gaq.push([ "_trackEvent", "Modal", "Close", $reference, null, !0 ]), 
        animated && Modernizr.cssanimations ? ($modalContent.addClass("fade-out-down"), 
        $modal.addClass("fade-out"), $("body").removeClass("modal-open"), $modal.one("webkitAnimationEnd oanimationend msAnimationEnd animationend", function() {
            $modal.remove(), modalIsOpen = !1;
        })) : ($("body").removeClass("modal-open"), $modal.remove(), modalIsOpen = !1);
    };
    ...
    return $(document).ready(function() {
        ...
        hash && "#/" !== hash && $(hash) && "text/cached-modal" === $(hash).attr("type") && open($(hash), !1);
    }), {
        isOpen: isOpen,
        open: open,
        close: close
...
define("neue/scroll-indicator", [], function() {
    ...
    function prepareIndicator(link) {
        ...
        var linkTarget = $(link).attr("href"), linkTargetOffset = $(linkTarget).offset().top, linkObj = {
            $el: $(link),
            targetOffset: linkTargetOffset
        };
        // Add jQuery object and offset value to links array
        links.push(linkObj), // Now that we're ready, let's calculate how stickies should be displayed
...
define("neue/validation", [ "require", "./events" ], function(require) {
   ...
   */
    ...
    return $("body").on("submit", "form", function(event, isValidated) {
        ...
        prepareFields($("body").find("[data-validate]")), Events.subscribe("Modal:opened", function(topic, args) {
...
}), define("campaign/sources", [ "jquery" ], function() {
    var $ = require("jquery"), $sources = $(".sources") || null;
    if ($sources) {
...
}), define("campaign/tips", [ "jquery" ], function() {
    var $ = require("jquery");
 ...
 */
define("utils/Features", [ "jquery" ], function() {
    var $ = require("jquery"), mediaQueries = function() {
        return window.matchMedia && null !== window.matchMedia("only all") && window.matchMedia("only all").matches;
    }, remUnits = function() {
        var div = document.createElement("div");
        return div.style.cssText = "font-size: 1rem;", /rem/.test(div.style.fontSize);
    }, _ie8 = $("html").hasClass("ie8"), ie8 = function() {
        return _ie8;
    }, _ie9 = $("html").hasClass("ie9"), ie9 = function() {
        return _ie9;
    };
    // Export API
    return {
...
}), define("campaign/ImageUploader", [ "require", "jquery", "utils/Features", "neue/events" ], function(require) {
    var $ = require("jquery"), Features = require("utils/Features"), Events = require("neue/events"), prepareImageUploadUI = function($context) {
        ...
        $imageUploads.each(function(i, el) {
            ...
            $(el).css("visibility", "hidden"), $(el).css("position", "absolute");
            ...
            $imgPreview.insertAfter($(el)), $imgPreview.hide();
            var $fileName = $("<p class='filename'></p>");
            $fileName.insertAfter($uploadBtn), $uploadBtn.on("click", function(event) {
                event.preventDefault(), $(el).trigger("click");
    ...
    Events.subscribe("Modal:opened", function(topic, args) {
        // @NOTE: We don't want to use this image upload UI on IE8 or IE9, since they
        // don't allow manipulation of input[type="file"] fields through JavaScript
        Features.ie8() || Features.ie9() || prepareImageUploadUI(args);
    });
});
...
var Kicksend = {
    mailcheck: {
        ...
        run: function(opts) {
            ...
            result ? opts.suggested && opts.suggested(result) : opts.empty && opts.empty();
        },
...
window.jQuery && !function($) {
...
}(this)), define("validation/auth", [ "require", "neue/validation", "mailcheck" ], function(require) {
    // # Helpers
    ...
    Validation.registerValidationFunction("reportbackNumber", function(string, done) {
        return done("" !== string && /^[,0-9]+$/.test(string) ? {
            success: !0,
   ...
   */
    Validation.registerValidationFunction("ups_address", function($el, done) {
        var $sorryError = $("<div class='messages error'><strong>We couldn't find that address.</strong> Double check for typos and try submitting again.</div>"), $networkError = $("<div class='messages error'>We're having trouble submitting the form, are you sure your internet connection is working? Email us if you continue having problems.</div>"), addressFieldData = $el.find("select, input").serializeArray();
...
define("Analytics", [ "neue/events" ], function(Events) {
    ...
    "undefined" != typeof _gaq && null !== _gaq && (Events.subscribe("Validation:InlineError", function(topic, args) {
...
require.config({
    ...
    shim: {
        ...
        mailcheck: {
            exports: "Kicksend.mailcheck"
        }
    }
}), /**
 * This is where we load and initialize components of our app.
 */
define("main", [ "require", "jquery", "jquery-once", "jquery-unveil", "finder/Finder", "neue/main", "campaign/sources", "campaign/tips", "campaign/ImageUploader", "validation/auth", "validation/reportback", "validation/address", "Analytics" ], function(require) {
    // Set $ global for jQuery
    var $ = require("jquery");
    // load in jQuery plugins
    require("jquery-once"), require("jquery-unveil");
    // let's get going
    var Finder = require("finder/Finder");
    // Initialize modules on load
    require("neue/main"), require("campaign/sources"), require("campaign/tips"), require("campaign/ImageUploader"), 
    require("validation/auth"), require("validation/reportback"), require("validation/address"), 
    require("Analytics"), $(function() {
        $(".js-finder-form").length && Finder.init($(".js-finder-form"), $(".js-campaign-results"), $(".js-campaign-blankslate"));
    });
}), require([ "main" ]);