...
function(jQuery) {
    ...
    }(jQuery.ajaxSettings.xdr()), // Create transport if the browser can provide an xdr
    jQuery.support.iecors && jQuery.ajaxTransport(function(s) {
        var xdr = s.xdr();
        return {
            send: function(headers, complete) {
                xdr.onload = function() {
                ...
                s.xhrFields && (xhr.onerror = s.xhrFields.error, xhr.ontimeout = s.xhrFields.timeout), 
                xdr.open(s.type, s.url), // XDR has no method for setting headers O_o
...
}(jQuery), define("jquery-iecors", function() {});
