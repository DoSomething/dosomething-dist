...
define("neue/scroll-indicator", [], function() {
   ...
   */
    ...
    function updateScrollIndicators() {
        var newIndicator = binarySearch(links, $(window).scrollTop() + 40), newIndicatorParents = newIndicator.link.parentsUntil(".js-scroll-indicator"), oldIndicatorParents = $();
...
define("neue/main", [ "require", "./carousel", "./events", "./jump-scroll", "./media-selector", "./menu", "./messages", "./modal", "./scroll-indicator", "./sticky", "./validation" ], function(require) {
    ...
    var $ = require("jquery"), _ = require("lodash"), Validation = require("neue/validation"), shouldValidateForm = !0, resultTpl = _.template("<li><a class='js-schoolfinder-result' data-gsid='<%- gsid %>' href='#'><span><%- name %></span> <em><%- city %>, <%- state %></em></a></li>"), getResults = function(state, query, callback) {
        var endpoint;
    ...
    }, displayResults = function($el, results) {
        $el.empty(), _.each(results, function(result) {
     ...
     */
        $schoolSearchField.on("keyup", function() {
            ...
            var val = $(this).val();
            "" !== val ? ($schoolSearchField.addClass("loading"), getResultsThrottled($stateField.val(), val, function(res) {
                displayResults($schoolSearchResults, res), $schoolSearchField.removeClass("loading");
   ...
   */
    Validation.registerValidationFunction("school_finder", function($el, done) {
        ...
        return shouldValidateForm && "" === $schoolID.val() ? ($el.addClass("shake"), $el.append("<div class='messages error'>You haven't entered your school. Find your school by entering your state and then selecting a school from the search results.</div>"), 
        done({
            success: !1
        })) : done({
            success: !0
 ...
 */
define("app", [ "require", "jquery", "finder/Finder", "neue/main", "campaign/SchoolFinder", "campaign/sources", "campaign/tips", "campaign/tabs", "campaign/ImageUploader", "validation/auth", "validation/reportback", "validation/address", "Analytics", "tiles" ], function(require) {
    ...
    require("Analytics"), require("tiles"), $(document).ready(function() {
