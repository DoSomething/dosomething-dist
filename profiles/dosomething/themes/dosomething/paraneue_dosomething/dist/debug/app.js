define("text", {
   ...
   */
     ...
     */
        init: function($div, queryCallback) {
            ...
            $div.find("[data-toggle]").click(function() {
                var $this = $(this), toggleClass = $this.data("toggle"), $parent = $this.parents("." + toggleClass), $siblings = $parent.siblings();
    ...
    */
    ...
    var $ = require("jquery"), Revealer = function($container, $content, category, initialItems, stepItems) {
        this.$container = $container, this.$content = $content, this.$button = $('<button class="button -tertiary">' + Drupal.t("Show More") + "</button>"), 
        this.category = category, this.initialItems = initialItems, this.stepItems = stepItems, 
    ...
    return Revealer.prototype.init = function() {
        "gallery" === this.category ? this.prepGalleryItems() : this.steps.items.push(this.$content), 
        this.steps.count = this.steps.items.length, this.detachContent(), this.enableButton();
        ...
        this.$button.on("click", function() {
            _this.steps.count > 0 && (_this.reattachContent(_this.steps.items.shift()), _this.steps.count--), 
            0 === _this.steps.count && _this.$button.remove();
        });
        ...
        if (this.$content.length > this.stepItems) for (;collection.length > 0; ) this.steps.items.push(collection.splice(0, this.stepItems)); else this.steps.items.push(collection);
        collection = null;
    }, Revealer;
}), define("swapper/Swapper", [], function() {
    var Swapper = function($content) {
        this.$image = $content.find("img"), this.init();
    };
    return Swapper.prototype.init = function() {
        var srcLarge = this.$image.data("src-large") || null;
        this.exchange(srcLarge);
    }, Swapper.prototype.exchange = function(url) {
        url && this.$image.attr("src", url);
    }, Swapper;
}), /**
 * @module neue/events
...
define("campaign/ImageCrop", [ "require", "jquery" ], function(require) {
    var $cropContainer, xInitPos, yInitPos, offsetLeft, offsetTop, initialCropBoxHeight, initialCropBoxWidth, previewImageWidth, previewImageHeight, minThreshold, $ = require("jquery"), resizeState = !1, dragState = !1, resizeClickPositionX = 0, resizeClickPositionY = 0, $resizeHandle = $("<div class='resize-handle'><div class='square'></div></div>"), $cropBox = $("<div>").addClass("cropbox").append($resizeHandle), getCoords = function(e) {
        var coords = {}, eventType = e.originalEvent.type;
        // Get the coordinates of a touchevent.
        return coords = "touchstart" === eventType || "touchmove" === eventType || "touchend" === eventType ? e.originalEvent.targetTouches[0] : {
            pageX: event.pageX,
            pageY: event.pageY
        };
    }, dragCropBoxInit = function() {
        // Bind touch and mouse events.
        $cropBox.on("touchstart mousedown", function(event) {
            event.preventDefault();
            var coords = getCoords(event);
            // Set the initial position of the mouse.
            xInitPos = coords.pageX, yInitPos = coords.pageY, // Sets the initial positioning of the crop box.
            offsetLeft = parseInt($cropBox.css("left"), 10), offsetTop = parseInt($cropBox.css("top"), 10), 
            // Turns on dragging.
            dragState = !0;
        });
    }, resizeCropBoxInit = function() {
        $(".resize-handle").on("touchstart mousedown", function(event) {
            event.preventDefault();
            // Grab the postioning of the event.
            var coords = getCoords(event);
            // Set the initial size of the crop box.
            initialCropBoxHeight = parseInt($cropBox.css("height"), 10), initialCropBoxWidth = parseInt($cropBox.css("width"), 10), 
            // Set the position of the mouse.
            resizeClickPositionX = coords.pageX, resizeClickPositionY = coords.pageY, // Turn on resizing.
            resizeState = !0;
        });
    }, dsCropperDestroy = function() {
        $cropBox.length && $cropBox.remove();
    }, dsCropperInit = function(origImage, $form) {
        // Get the original width of the image.
        var origImageWidth = origImage.width, $previewImage = $("#modal--crop").find(".preview");
        // Make sure there is a preview image on the page before proceeding.
        if ($previewImage.length) {
            // Get the hidden form elements.
            var $cropX = $form.find("input[name='crop_x']"), $cropY = $form.find("input[name='crop_y']"), $cropWidth = $form.find("input[name='crop_width']"), $cropHeight = $form.find("input[name='crop_height']");
            // Wrap the image in a container and get it's dimensions to use as a boundary.
            $previewImage.wrap($("<div class='crop-bounding-box'></div>")), $cropContainer = $(".crop-bounding-box"), 
            previewImageWidth = $cropContainer.width(), previewImageHeight = $cropContainer.height(), 
            // Calculate the initial dimensions of the cropbox. Add the cropbox to the page.
            $cropBox.width(previewImageHeight / 2), $cropBox.height(previewImageHeight / 2), 
            minThreshold = previewImageHeight / 2, $cropBox.insertBefore($previewImage), // Initiate dragging and resizing.
            dragCropBoxInit(), resizeCropBoxInit(), // Bind move events to the document.
            $cropBox.on("touchmove mousemove", function(event) {
                event.preventDefault();
                var coords = getCoords(event), cropBoxHeight = parseInt($cropBox.css("height"), 10), cropBoxWidth = parseInt($cropBox.css("width"), 10), cropBoxTop = parseInt($cropBox.css("top"), 10), cropBoxLeft = parseInt($cropBox.css("left"), 10);
                // If dragging, drag the crop box along.
                if (dragState) {
                    // Calculate how far to move the box down and right based on the mouse position.
                    var moveRight = offsetLeft + (coords.pageX - xInitPos), moveDown = offsetTop + (coords.pageY - yInitPos), withinTopBounds = previewImageHeight - cropBoxHeight > moveDown ? !0 : !1, withinLeftBounds = previewImageWidth - cropBoxWidth > moveRight ? !0 : !1;
                    // Make sure we are still within the bounds of the image and then actully move the box.
                    withinTopBounds && moveDown > 0 && $cropBox.css("top", moveDown), withinLeftBounds && moveRight > 0 && $cropBox.css("left", moveRight);
                }
                // If resizing, resize the crop box.
                if (resizeState) {
                    dragState = !1;
                    // Calculate how much to resize the cropbox.
                    var resizeHorizontal = initialCropBoxWidth + (coords.pageX - resizeClickPositionX), resizeVertical = initialCropBoxHeight + (coords.pageX - resizeClickPositionX), withinHeightBounds = previewImageHeight > cropBoxHeight + cropBoxTop ? !0 : !1, withinWidthBounds = previewImageWidth > cropBoxWidth + cropBoxLeft ? !0 : !1, cropBoxSizeAllowed = cropBoxWidth > minThreshold ? !0 : !1, increasingSize = coords.pageX - resizeClickPositionX > 0 ? !0 : !1, decreasingSize = coords.pageX - resizeClickPositionX < 0 ? !0 : !1;
                    // Make sure we are still in the boundaries of the image, and then resize the box.
                    withinHeightBounds && withinWidthBounds && (cropBoxSizeAllowed || increasingSize) ? ($cropBox.css("height", resizeVertical), 
                    $cropBox.css("width", resizeHorizontal)) : decreasingSize && cropBoxSizeAllowed && ($cropBox.css("height", resizeVertical), 
     ...
     */
            $cropBox.on("touchend mouseup", function() {
                dragState = !1, resizeState = !1;
                // Calulate how much the image has been resized.
                var resizeAmount = origImageWidth / $previewImage.width(), finalCrop = {
                ...
                // populate fields.
                $cropX.val(finalCrop.left), $cropY.val(finalCrop.top), $cropWidth.val(finalCrop.width), 
...
define("campaign/ImageUploadBeta", [ "require", "jquery", "neue/modal", "campaign/ImageCrop" ], function(require) {
    var $ = require("jquery"), Modal = require("neue/modal"), ImageUploadBeta = function($button) {
        this.$uploadButton = $button, this.$cropModal = $("#modal--crop"), this.$reportbackForm = $("#dosomething-reportback-form"), 
        this.$imageForm = this.$cropModal.find("#dosomething-reportback-image-form"), this.$imagePreview = this.$cropModal.find(".image-preview"), 
        this.imageValues = {}, this.cropEnabled = "undefined" != typeof Drupal.settings.dsReportback.cropEnabled ? Drupal.settings.dsReportback.cropEnabled : !1, 
        this.init();
    };
    /**
   ...
   */
        ...
        if ($imageContainer.html($preview), /^image/.test(files[0].type)) {
            ...
            reader.onloadend = function() {
                ...
                $preview.attr("src", result).load(function() {
                    ...
                    _this.cropEnabled && _this.cropInit(result);
                });
            };
        }
    }, ImageUploadBeta.prototype.cropInit = function(result) {
        var ImageCrop = require("campaign/ImageCrop"), origImage = new Image();
        origImage.src = result, // Remove any cropping already on the page before initiating.
        // @TODO - remove this check. the crop init js should be smart enough to reset itself it is already initiate.
        ImageCrop.dsCropperDestroy(), ImageCrop.dsCropperInit(origImage, this.$reportbackForm);
    }, ImageUploadBeta.prototype.getImageCaption = function() {
        return this.$imageForm.find("#caption").val();
    }, ImageUploadBeta;
}), define("text!reportback/templates/reportback.tpl.html", [], function() {
    ...
    var $ = require("jquery"), _ = require("lodash"), Events = require("neue/events"), ImageUploadBeta = require("../campaign/ImageUploadBeta"), reportbackTplSrc = require("text!reportback/templates/reportback.tpl.html"), Reportback = {
        $initialGallery: null,
     ...
     */
        init: function($container) {
            var _this = this;
            this.reportbackContainer = $container, this.$reportbackWrapper = this.reportbackContainer.find(".wrapper"), 
            this.$reportbackForm = this.reportbackContainer.find("#dosomething-reportback-form"), 
            this.$uploadButton = this.reportbackContainer.find(".js-image-upload-beta"), this.$initialGallery = $container.find(".gallery--reportback"), 
            this.itemsPrefetched = $container.data("prefetched"), this.itemsTotal = $container.data("total") - this.itemsPrefetched, 
            this.nid = $container.data("nid"), this.offset = this.itemsPrefetched, this.itemsPrefetched < this.itemsTotal && (this.apiUrl = this.buildApiUrl(), 
            this.enableViewMore(), this.enableResponsive());
     ...
     */
        appendEntries: function(inventoryIndex) {
            var start = inventoryIndex, end = start + this.bundleSize, cluster = "";
            this.outOfStock(start) && (end = this.itemsTotal);
            for (var i = start; end > i; i++) void 0 !== this.inventory[i] && (cluster += this.inventory[i]);
            this.galleryInDom ? this.$extendedGallery.append(cluster) : this.insertGallery(cluster);
        },
        /**
     ...
     */
        enableViewMore: function() {
            var _this = this;
            this.$viewMoreContainer = $('<div class="view-more"></div>'), this.$viewMoreButton = $('<button id="view-more-button" class="btn btn--view-more tertiary">View more</button>'), 
            this.$viewMoreContainer.append(this.$viewMoreButton), this.$viewMoreContainer.insertAfter(this.isLargeMedia() ? this.$reportbackWrapper : this.$initialGallery), 
     ...
     */
        enableResponsive: function() {
            ...
            $(window).on("resize", _.debounce(function() {
                _this.isLargeMedia() ? _this.$viewMoreContainer && (_this.$viewMoreContainer.detach(), 
                _this.$viewMoreContainer.insertAfter(_this.$reportbackWrapper)) : _this.$viewMoreContainer && (_this.$viewMoreContainer.detach(), 
     ...
     */
        evaluateInventory: function() {
            this.inventory.length ? this.lowInventory() ? this.outOfStock(this.currentIndex) ? (this.appendEntries(this.currentIndex), 
            this.currentIndex = this.inventory.length) : (this.request(this.currentIndex), this.currentIndex += this.bundleSize) : (this.appendEntries(this.currentIndex), 
            this.currentIndex += this.bundleSize) : (this.request(this.currentIndex), this.currentIndex += this.bundleSize), 
            this.currentIndex === this.itemsTotal && this.disableViewMore();
        },
        /**
     ...
     */
        insertGallery: function(data) {
            ...
            content && this.$extendedGallery.html(content), this.$viewMoreContainer.prepend(this.$extendedGallery), 
            this.galleryInDom = !0;
        },
        /**
     ...
     */
        lowInventory: function() {
            return this.currentIndex + this.bundleSize > this.inventory.length ? !0 : !1;
        },
        /**
     ...
     */
        outOfStock: function(quantity) {
            return quantity + this.bundleSize > this.itemsTotal ? !0 : !1;
        },
        /**
     ...
     */
        request: function(inventoryIndex) {
            ...
            $.ajax({
                ...
                _this.templatize(response), _this.appendEntries(inventoryIndex);
     ...
     */
        templatize: function(items) {
            for (var i = 0, count = items.length; count > i; i++) {
                var data = {
                    id: items[i].rbid,
                    image: items[i].src,
                    ...
                    isListItem: !0
                }, markup = _.template(reportbackTplSrc, data);
...
define("neue/carousel", [], function() {
    ...
    $(function() {
        ...
        function showCurrentSlide(direction) {
            ...
            $("#slide" + counter).removeClass("is-visible"), // Increment or decrement slide position based on user"s request
        ...
        var counter = 0, totalCount = $(".carousel__slide").length - 1, $buttons = $("#prev, #next");
...
define("neue/jump-scroll", [], function() {
...
}), define("neue/media-options", [], function() {
    var $ = window.jQuery, MediaOptionsSelector = function($fieldGroup, options) {
        if (void 0 !== $fieldGroup && $($fieldGroup.length)) {
            ...
            options = options || {}, _this.options = options = {
                fieldClassName: "string" == typeof options.fieldClassName ? options.fieldClassName : "media-options",
                fieldSelector: "string" == typeof options.fieldSelector ? options.fieldSelector : ".-radio",
                optionSelector: "string" == typeof options.optionSelector ? options.optionSelector : "label"
            }, _this.$fieldGroup = $($fieldGroup).addClass(_this.options.fieldClassName), _this.$checked = [], 
            _this.init();
        }
    };
    ...
    */
        check: function($field) {
            $field.addClass("is-selected").find("input[type='radio']").attr("checked", !0), 
            this.$checked = $field;
        },
        /**
    ...
    */
        uncheck: function($field) {
            $field.removeClass("is-selected").find("input[type='radio']").attr("checked", !1);
        }
    }, $(function() {
...
define("neue/menu", [], function() {
    ...
    $(function() {
        ...
        $(".js-navigation-toggle").on("touchstart mousedown", function(e) {
            e.preventDefault(), $(".chrome").toggleClass("mobile-menu-open"), $(".navigation").toggleClass("is-visible");
        });
        // Hide footer on mobile until clicked
        var $collapsible = $(".js-toggle-collapsed");
        $collapsible.addClass("is-collapsed is-toggleable"), $collapsible.on("click", function() {
            window.matchMedia("screen and (max-width: 768px)").matches && $(this).toggleClass("is-collapsed");
        });
    });
}), /**
 ...
 */
define("neue/main", [ "require", "./carousel", "./events", "./jump-scroll", "./media-options", "./menu", "./messages", "./modal", "./scroll-indicator", "./sticky", "./validation" ], function(require) {
 ...
 */
define("app", [ "require", "jquery", "lodash", "finder/Finder", "donate/Donate", "revealer/Revealer", "swapper/Swapper", "reportback/Reportback", "neue/main", "campaign/SchoolFinder", "campaign/sources", "campaign/tips", "campaign/tabs", "campaign/ImageUploader", "validation/auth", "validation/reportback", "validation/address", "validation/donate", "Analytics", "tiles" ], function(require) {
    var $ = require("jquery"), _ = require("lodash"), Finder = require("finder/Finder"), Donate = require("donate/Donate"), Revealer = require("revealer/Revealer"), Swapper = require("swapper/Swapper"), Reportback = require("reportback/Reportback");
     ...
     */
        if ($revealGalleries.length > 0) {
            ...
            $revealGalleries.each(function(index, gallery) {
                var $gallery = $(gallery), $tiles = $gallery.find("li"), isMosaicGallery = $gallery.hasClass("-mosaic"), initialItems = isMosaicGallery ? 5 : 6, stepItems = isMosaicGallery ? 8 : 6;
                $tiles.length > initialItems && (galleryList[index] = new Revealer($gallery, $tiles, "gallery", initialItems, stepItems));
            });
        }
     ...
     */
        if ($mosaicGalleries.length && $mosaicGalleries.hasClass("-featured")) {
            ...
            $(window).on("resize", _.debounce(function() {
                window.matchMedia("(min-width: 768px)").matches && !swappedImagesList.length && $mosaicGalleries.each(function(index) {
     ...
     */
        ...
        $reportback.length && Reportback.init($reportback), $("html").addClass("js-ready");
    });
}), require([ "app" ]);