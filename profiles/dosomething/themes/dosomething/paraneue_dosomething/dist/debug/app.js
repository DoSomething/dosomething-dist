   ...
   */
    return ImageUploadBeta.prototype.init = function() {
        ...
        this.$uploadButton.on("change", function(event) {
            ...
            _this.previewImage(files, _this.$imagePreview), // Check if cropping is enabled, and then intiate.
            _this.cropEnabled && _this.cropInit(files));
        }), /**
     ...
     */
        ...
        var ImageCrop = require("campaign/ImageCrop"), _this = this;
        if (/^image/.test(files[0].type)) {
            var reader = new FileReader();
            reader.readAsDataURL(files[0]), reader.onloadend = function() {
                var origImage = new Image();
                origImage.src = this.result, // Remove any cropping already on the page before initiating.
                ImageCrop.dsCropperDestroy(), ImageCrop.dsCropperInit(origImage, _this.$reportbackForm);
            };
        }
