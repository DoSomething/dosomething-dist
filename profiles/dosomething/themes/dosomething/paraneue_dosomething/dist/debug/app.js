...
define("campaign/ImageUploadBeta", [ "require", "jquery", "neue/modal" ], function(require) {
    var $ = require("jquery"), Modal = require("neue/modal"), ImageUploadBeta = function($button) {
        this.$uploadButton = $button, this.$cropModal = $("#modal--crop"), this.$imageForm = this.$cropModal.find("#dosomething-reportback-image-form"), 
        this.$imagePreview = this.$cropModal.find(".image-preview"), this.imageValues = {}, 
        this.init();
    };
    /**
   ...
   */
    return ImageUploadBeta.prototype.init = function() {
        ...
        this.$uploadButton.on("change", function(event) {
            ...
            if (files.length && window.FileReader) {
                ...
                if (_this.$imagePreview.html($preview), /^image/.test(files[0].type)) {
...
}), define("campaign/Reportback", [ "require", "jquery", "neue/events", "campaign/ImageUploadBeta", "revealer/Revealer" ], function(require) {
    var $ = require("jquery"), Events = require("neue/events"), ImageUploadBeta = require("campaign/ImageUploadBeta"), Reportback = (require("revealer/Revealer"), 
    {
     ...
     */
        init: function($container) {
            ...
            this.$viewMoreButton.insertAfter($initialGallery), this.$viewMoreButton.on("click", this.request);
            // Initializes image upload modal.
            var imageUpload = new ImageUploadBeta(this.$uploadButton), _this = this;
...
define("campaign/ImageCrop", [ "require", "jquery" ], function(require) {
    ...
    }, dsCropperInit = function(origImage) {
        // Get the original width of the image.
        var origImageWidth = origImage.width, $previewImage = $("#modal-report-back").find(".preview");
        // Make sure there is a preview image on the page before proceeding.
        $previewImage.length && (// Wrap the image in a container and get it's dimensions to use as a boundary.
        $previewImage.wrap($("<div class='image-crop-container'></div>")), $cropContainer = $(".image-crop-container"), 
        previewImageWidth = $previewImage.width(), previewImageHeight = $previewImage.height(), 
     ...
     */
        $cropContainer.on("touchend mouseup", function() {
            ...
            var resizeAmount = origImageWidth / $previewImage.width(), $cropX = $("#dosomething-reportback-form").find("input[name='crop_x']"), $cropY = $("#dosomething-reportback-form").find("input[name='crop_y']"), $cropWidth = $("#dosomething-reportback-form").find("input[name='crop_width']"), $cropHeight = $("#dosomething-reportback-form").find("input[name='crop_height']");
            dragState = !1, resizeState = !1, // Calculate cropping information, taking into account how much the image has been scaled.
            finalCrop = {
    ...
    $(document).ready(function() {
        ...
        cropEnabled && $(".js-image-upload").on("change", function(event) {
            event.preventDefault();
            var fileReader = new FileReader();
            fileReader.onload = function() {
                var origImage = new Image();
                origImage.src = fileReader.result, dsCropperInit(origImage);
 ...
 */
define("app", [ "require", "jquery", "finder/Finder", "donate/Donate", "revealer/Revealer", "swapper/Swapper", "campaign/Reportback", "neue/main", "campaign/SchoolFinder", "campaign/sources", "campaign/tips", "campaign/tabs", "campaign/ImageUploader", "campaign/ImageCrop", "validation/auth", "validation/reportback", "validation/address", "validation/donate", "Analytics", "tiles" ], function(require) {
    var $ = require("jquery"), Finder = require("finder/Finder"), Donate = require("donate/Donate"), Revealer = require("revealer/Revealer"), Swapper = require("swapper/Swapper"), Reportback = require("campaign/Reportback");
    ...
    require("campaign/ImageCrop"), require("validation/auth"), require("validation/reportback"), 
    require("validation/address"), require("validation/donate"), require("Analytics"), 
    require("tiles"), $(document).ready(function() {
