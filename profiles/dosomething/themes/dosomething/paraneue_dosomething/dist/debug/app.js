...
define("neue/main", [ "require", "./carousel", "./events", "./jump-scroll", "./media-selector", "./menu", "./messages", "./modal", "./scroll-indicator", "./sticky", "./validation" ], function(require) {
    ...
    Validation.registerValidationFunction("birthday", function(string, done) {
        var birthday, birthMonth, birthDay, birthYear, format = "MM/DD/YYYY";
        // parse date from string
        if (null != Drupal.settings.dsValidation && null != Drupal.settings.dsValidation.dateFormat && (format = Drupal.settings.dsValidation.dateFormat), 
        "MM/DD/YYYY" === format && /^\d{1,2}\/\d{1,2}\/\d{4}$/.test(string)) // US formatting
